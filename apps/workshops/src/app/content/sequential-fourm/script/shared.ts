...
export const BEFORE_DOORS: L[] = [
  ...
  { en: `<strong>On the loaner units, go through the GLOBAL menu.</strong> Each of these breaks an exercise if it is set the other way:<br>· <strong>Pot Mode = Jump</strong> (Global 4). The one that matters most. Every exercise says &#x201C;cutoff to 9 o&#x27;clock&#x201D;; on any other setting the knob does nothing until it reaches the value stored in the program, and the person turning it thinks they broke the synth.<br>· <strong>Local Control = On</strong> (Global 3). On Off the keyboard stops talking to the synth: keys press, nothing sounds.<br>· <strong>Aftertouch = Poly</strong> (Global 9) and <strong>Atouch Curve = Medium</strong> (Global 10). Module 07 is about the pressure on each separate key; on mono there is nothing to show. Medium is the straight curve.<br>· <strong>Velocity Curve = Medium</strong>, for the same reason.<br>· <strong>Clock Mode = Out or Off</strong>. On In or In/Thru the arpeggiator waits for a clock from outside and simply never starts, which stops module 08 dead.<br>· <strong>Alt Scale = Equal Temperament</strong>, <strong>Coarse Tune</strong> and <strong>Fine Tune = 0</strong>, so every instrument in the room is tuned the same.<br>· <strong>Tune Voices</strong>, after fifteen minutes of warm-up.<br>Keybed firmware: <strong>keymech OS 1.0.0.2 or newer</strong>. On older versions the lowest seven keys send maximum aftertouch by themselves. After any update run the aftertouch calibration, then press into those low keys and check that they respond gradually.`, ro: `<strong>Pe unitățile împrumutate, treci prin meniul GLOBAL.</strong> Fiecare dintre setările astea strică un exercițiu dacă e pusă invers:<br>· <strong>Pot Mode = Jump</strong> (Global 4). Cea mai importantă. Toate exercițiile spun „cutoff la ora nouă&#x201D;; pe orice altă setare knob-ul nu face nimic până nu ajunge la valoarea salvată în program, iar omul care îl rotește crede că a stricat synth-ul.<br>· <strong>Local Control = On</strong> (Global 3). Pe Off, claviatura nu mai vorbește cu synth-ul: apeși clape și nu iese niciun sunet.<br>· <strong>Aftertouch = Poly</strong> (Global 9) și <strong>Atouch Curve = Medium</strong> (Global 10). Modulul 07 e despre presiunea pe fiecare clapă în parte; pe mono nu are ce arăta. Medium e curba dreaptă.<br>· <strong>Velocity Curve = Medium</strong>, din același motiv.<br>· <strong>Clock Mode = Out sau Off</strong>. Pe In sau In/Thru, arpeggiatorul așteaptă un clock din afară și pur și simplu nu pornește, deci modulul 08 se blochează.<br>· <strong>Alt Scale = Equal Temperament</strong>, <strong>Coarse Tune</strong> și <strong>Fine Tune = 0</strong>, ca toate instrumentele din sală să fie acordate la fel.<br>· <strong>Tune Voices</strong>, după cincisprezece minute de încălzire.<br>Firmware-ul claviaturii: <strong>keymech OS 1.0.0.2 sau mai nou</strong>. Pe versiunile vechi, cele mai joase șapte clape trimit singure aftertouch la maximum. După orice update rulează calibrarea de aftertouch, apoi apasă în clapele alea de jos și verifică dacă răspund gradual.` },
  ...
  { en: `student handbooks, if they are printed: one on every station (RO; EN on request), the rest by the exit; water; photo/video.`, ro: `manualele cursantului, dacă sunt tipărite: câte unul pe stație (RO; EN la cerere), restul la ieșire; apă; foto/video.` },
...
export const PLAN_B: L[] = [
  { en: `<strong>No projector:</strong> teach from the panel; if the handbook is printed, its pages follow the slide order; overhead camera on a TV, if one exists.`, ro: `<strong>Fără proiector:</strong> predai de la panou; dacă manualul e tipărit, paginile lui urmează ordinea planșelor; overhead camera pe un TV, dacă există.` },
...
export const TRAPS: L[] = [
  ...
  { en: `<strong>Aftertouch:</strong> say what is certainly true, „press harder → brighter (FILT) / bend (FREQ), on the pressed note&#x201D;, and leave what the LFO shares between voices to the Fourm manual.`, ro: `<strong>Aftertouch:</strong> spune ce e sigur adevărat, „apasă mai tare → mai strălucitor (FILT) / bend (FREQ), pe nota apăsată”, iar ce împarte LFO-ul între voci lasă-l pe seama manualului Fourm.` },
...
export const CHEAT_ROWS: CheatRow[] = [
  ...
  },
  {
    ...
    theyDo: { en: `From Basic Program (hold <strong>transpose down</strong> + <strong>program</strong>), rebuild the bass from 04 from memory: A builds, B watches the panel and prompts; then the other way. Settings get written down.`, ro: `Din Basic Program (hold <strong>transpose down</strong> + <strong>program</strong>), reconstruiți bass-ul de la 04 din memorie: A construiește, B se uită pe panou și îl ajută; apoi invers. Pozițiile se notează.` },
    who: { en: `A → swap → B`, ro: `A → swap → B` },
  },
];
